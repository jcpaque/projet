...
for($i=2;$i<=$max;$i++)
{
	$normal1jour[$i][1]=($tab2[$i][2]-$tab2[$i-1][2])/($normal1jour[$i][0]-$normal1jour[$i-1][0]);
}
// echo '<pre>';
// print_r($normal1jour);
// echo '</pre>';
...
for($i=7;$i<=$max;$i++)
{
	$pentenormal1[$i][1]=($tab2[$i][2]-$tab2[$i-6][2])/($pentenormal1[$i][0]-$pentenormal1[$i-6][0]);
}
// echo '<pre>';
...
$_SESSION['penteNulleNormal2']=$sommeJours+1;
