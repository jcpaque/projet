...
for($i=2;$i<=$max;$i++)
{
	$normal1jour[$i][1]=($tab2[$i][4]-$tab2[$i-1][4])/($normal1jour[$i][0]-$normal1jour[$i-1][0]);
}
// echo '<pre>';
// print_r($normal1jour);
// echo '</pre>';
...
for($i=7;$i<=$max;$i++)
{
	$pentenormal1[$i][1]=($tab2[$i][4]-$tab2[$i-6][4])/($pentenormal1[$i][0]-$pentenormal1[$i-6][0]);
}
// echo '<pre>';
...
$_SESSION['penteNulleExtra2']=$sommeJours+1;
